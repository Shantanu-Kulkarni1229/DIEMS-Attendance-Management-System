...
export default function MarkAttendanceModal({ onClose, initialData, onSaved, dashboardData }) {
  ...
  useEffect(() => {
    ...
    loadStudents();
  }, [selectedClassroomId, studentsByClassroom, useDemo]);

  // when toggling demo mode, preselect demo ids or restore defaults
  useEffect(() => {
    if (useDemo) {
      setSelectedClassroomId('demo-tya');
      setSelectedSubjectId('demo-cp');
    } else {
      if (classrooms[0]) setSelectedClassroomId(classrooms[0]._id);
      if (subjects[0]) setSelectedSubjectId(subjects[0]._id);
    }
  }, [useDemo]);

  // keep endTime one hour after startTime unless user modified endTime separately
  useEffect(() => {
    if (!startTime) return;
    const [h, m] = (startTime || '09:00').split(':').map(Number);
    const dateObj = new Date();
    dateObj.setHours(h + 1, m, 0, 0);
  ...
  const resetAll = () => { setStudents((prev) => prev.map((s) => ({ ...s, present: true }))); setHighlightedRows([]); };

  const saveAttendance = async () => {
    if (!selectedClassroomId || !selectedSubjectId || !date) { setMessage({ type: 'error', text: 'Please select class, subject, and date.' }); return; }
    if (!students.length) { setMessage({ type: 'error', text: 'No students found for selected class.' }); return; }
    ...
    try {
      if (useDemo) {
        ...
        setMessage({ type: 'success', text: 'Attendance saved (demo mode).' });
          ...
          {loading ? <p className="text-sm text-slate-500">Loading classes and subjects...</p> : (
            <>
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="text-sm text-slate-600">{selectedClassroom ? `Class: ${selectedClassroom.name}` : 'No class selected'}{selectedSubject ? ` | Subject: ${selectedSubject.name}` : ''}</div>
                <div className="flex gap-4 text-sm font-semibold"><div className="text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">Present: {presentCount}</div><div className="text-red-600 bg-red-50 px-3 py-1 rounded-full">Absent: {absentCount}</div></div>
              </div>
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {students.map((student) => (
                      <tr key={student.id} className={`hover:bg-slate-50 ${!student.present ? 'bg-red-50/30' : ''}`} onClick={() => toggleAttendance(student.id)}>
                    ...
                    {students.length === 0 && (<tr><td colSpan={3} className="px-6 py-6 text-center text-slate-500">No students found for selected classroom.</td></tr>)}
                  </tbody>
                </table>
              </div>
            </>
