export default function Navbar({ setSidebarOpen, sidebarOpen, profile }) {
  ...
  const displayRole = profile?.className ? `${profile.className}${profile.division ? ` - ${profile.division}` : ''}` : 'Learner';
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 flex-1">
          ...
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 overflow-hidden flex items-center justify-center">
              <img src="/image.png" alt="DIEMS logo" className="w-full h-full object-contain p-1" />
            </div>
            <div className="hidden sm:block">
              <p className="text-sm font-semibold text-slate-800 leading-none">DIEMS</p>
              <p className="text-[11px] text-slate-500 font-medium mt-1">Attendance System</p>
            </div>
          </div>
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <div className="flex items-center gap-3 pl-4 md:border-l border-slate-200 cursor-pointer group">
            <div className="w-10 h-10 rounded-full bg-linear-to-br from-blue-600 to-sky-500 text-white flex items-center justify-center font-bold shadow-md ring-2 ring-white group-hover:shadow-lg transition-shadow">
              {initials}
            </div>
